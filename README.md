...
Тесты работают без интернета и настоящих ключей. Проверяются фильтры и их границы, 0/1/2/3 карточки, все три исхода, повреждённый CSV, повторные id, тайм-аут и ошибки AI, локальные цитаты, неизменность порядка при перезапуске, смене регистра и режима объяснений. GitHub Actions отключён: тесты и локальное демо запускаются в рабочей среде без расходов на GitHub. На 23.09.2026 локально прошли 40 тестов. Подробности — в файле состояния проекта.
...
В папке [`web/`](web/README.md) находится интерфейс с рекомендациями, списком остальных подрядчиков и проверенными предложениями близких дат, похожих форматов и бюджета.
...
Откройте http://127.0.0.1:4317. Веб-версия работает локально без AI-ключа. Цены указаны «от» по каталогу; подробного прайса и расчётной верхней цены нет. Особенности синхронизации, ограничения и примеры описаны в `web/README.md`.
